...
def _process_xml(
    ...
    if ckpt.is_dead_letter(fingerprint):
        log.debug("xml_skipped_dead_letter", extra={"source": _SOURCE_NAME, "file": str(it.file_path)})
        return None
    ...
    try:
        ...
        _handle_dead_letter(ckpt, fingerprint, str(it.file_path), exc, _SOURCE_NAME)
